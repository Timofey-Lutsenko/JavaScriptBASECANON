...
function clickHandler(event) {
    prodsDiv.textContent = ''; //вам нужно очищать содержимое .products
    showCategory(event.dataset.type); //в showCategory надо передать строку с типом категории, тип берите
...
function showCategory(category) {
    switch (category) {
        case 'phones':
            Object.values(products.phones).forEach(currEl => prodsDiv.insertAdjacentHTML('afterbegin', (getProductMarkup(currEl))));
            break;

        case 'tablets':
            Object.values(products.tablets).forEach(currEl => prodsDiv.insertAdjacentHTML('afterbegin', (getProductMarkup(currEl))));
            break;

        case 'tv':
            Object.values(products.tv).forEach(currEl => prodsDiv.insertAdjacentHTML('afterbegin', (getProductMarkup(currEl))));
            break;
    }
}
...
let targetBtn = Array.from(document.querySelectorAll('button'));
let prodsDiv = document.querySelector('div');

targetBtn.forEach(currBtn => currBtn.addEventListener('click', function (event) {
